...
- **知识沉淀**：`docs/solutions/` 存放 ce-compound 等流程落盘的已解问题与约定（按子目录分类，YAML frontmatter 含 `module`、`tags`、`problem_type` 等）；涉及与既有沉淀重叠的实现或排错时，可在该目录检索。
...
本项目使用 Cursor Canvas 作为「人类文档」的入口和交付物。

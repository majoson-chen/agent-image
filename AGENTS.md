...
### Cursor Canvas（与计划、实现对齐）

若当前工作在 **Cursor** 且环境提供 **Canvas**（典型信号：存在 **`canvas`** / **`docs-canvas`** 等技能说明与 IDE 侧 **`.canvas.tsx`** / 托管 **`canvases/`** 惯例），应把 Canvas **作为常规辅助面**使用：**人类侧导读与对齐层**，与本仓库既定的计划文档、需求 / 设计说明及正在变动的源码**语义一致**。环境不具备 Canvas 时本条不适用，勿套用。

- **对齐（默认）：** 出现新的架构视图、产品线、分层 / 边界或与本画布主题**相交**的实现变更时，**顺带评估**是否要新建或更新相关 `.canvas.tsx`。**执行 `/ce-compound`（或等价「沉淀解法」流程）收尾前**，多一份心眼：compound 内容与某张画布若讲同一脉络，画布是否仍为真；若脱节，顺带改画布或在回复里点明跟进项。
- **不同步的代价（须有意识）：** 过期的画布会把**错误的心智模型**写给人类与后续会话——排错按图索骥却对不上代码、在错误边界上继续 `ce-plan`/评审、反复解释「文档里不是这样」；信任与节奏一起赔掉。把画布当作**会腐烂的界面**：与代码一起变，别留静态谎言。
- **权威：** Canvas **不顶替**已定稿的计划、需求条文与源码；追溯仍以仓库 Markdown 与代码为准。
- **做法：** Agent 按需阅读宿主提供的「**canvas** / **docs-canvas**」skill 全文即可（单文件、`cursor/canvas`、禁止网络调用等均以技能正文为准）；勿依赖本小节再叠一层死板核查表。**CE：** 若在 `ce-plan` 中写了画布相关 Implementation Unit，`ce-work` 按单元的 Verification 收口即可——纯属组织手段，非硬门槛。
- **回复里一句话（有则必说，宜短）：** 若**已**改或同步了画布，用一行点明（例：`画布已按本次变更更新：…`）；若**应**改但本轮未动、存在明显漂移风险，也用一行点明（例：`提示：架构画布可能已过期，建议下一轮改 … 或与某 PR/计划对齐后更新`）。不写长文，但必须露信号，避免静默失步。
- **边界：** Canvas **不是**应用内的业务路由 / 页面；勿用本项目业务侧的 import 别名与框架源码布局去拼装画布。**拷贝到 Cursor Rules 时：** 可自行删去下一句。**本仓库：** 根目录 `canvases/` 为可选版本源（与 TS/ESLint 排除配置配套）；画布内容写好后按需自行同步至 IDE 托管的 `canvases/`（以 **`canvas`** skill 中的路径约定为准）。

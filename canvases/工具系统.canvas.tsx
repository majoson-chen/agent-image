        ...
        <Stack gap={24} style={{ padding: 24 }}>
            ...
            <Grid columns={2} gap={12}>
                ...
                <Card>
                    ...
                    <CardBody>
                        <Stack gap={4}>
                            <Text size="small">生图 tool 在 `execute` 中调用 `executeImageGeneration()`（`lib/providers/registers/_shared/image-execute/execute.server.ts`），经 **Catalog IMAGE 行 `executeImageGeneration`（image.execution）** 派发至各 Register HTTP/落盘逻辑。返回 `imageId` 等。`params.size` 在工具注册时从 `ConversationModelSelection.params` 读取，未配置则取 `capabilities.supportedSizes[0]`。</Text>
            ...
            <div style={{ padding: '12px 16px', border: `1px solid ${theme.stroke.tertiary}`, borderRadius: 6 }}>
                <Stack gap={8}>
                    <Text size="small">
                        <Text weight="semibold" as="span">绑定驱动工具</Text>
                        的 `apiKey` 在 `buildAvailableTools()` 内部实时查询 `SearchToolBinding → Model`，注册时注入工具工厂函数。绑定删除后下次请求工具即消失。
                    </Text>
                    <Text size="small">
                        <Text weight="semibold" as="span">选型驱动工具</Text>
                        的 `model` 记录和 `params.size` 在 `buildAvailableTools()` 内部实时查询 `ConversationModelSelection → Model`，注册时注入 `createImageGenerateTool()`。选型切换或清除后下次请求立即生效。
                    </Text>
                </Stack>
            </div>
        </Stack>

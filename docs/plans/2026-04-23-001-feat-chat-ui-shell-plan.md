...
在 **`design/layout.pen`** 中交付 **agent-image 对话主界面** 的 **线框级设计稿**（非营销插画）：覆盖 **窄侧栏、主工作区、三档模型选择、用量可视化（有则显 / 无则隐）、消息与时间线、可折叠工具步骤、生图前确认（R15）、工具失败可见（R16）、Provider / Search 设置入口（F1 / R17）** 等信息的 **布局、层级与关键状态**。  
全文案面向 **中文** 用户，可用占位句。
...
单机玩具级产品需要「类 Cursor」的 **多步可见** 对话体验。用户须在 **对话视图** 中完成 **LLM / 主生图 / 可选次生图** 选择（R3）、在 **有数据时** 看到 **用量累计**（R6）、在 **生图前** 通过简明 **「是否允许 …」** 确认（R15）、并 **看到工具失败**（R16）。  
...
2. **主栏顶部**：一行 **LLM | 主生图 | 次生图（可空）** + **用量条**（R6：有累计则呈现整块；无数据则 **整区不显** 或等价处理，稿中可各做一帧说明）。
3. **主栏中部**：用户消息、助手消息、**可折叠工具步骤**（工具名 + 进行中 / 成功 / 失败）、**对话内图片占位**（R18：稿中用矩形/示意即可）。
4. **主栏底部**：输入区、主发送动作；**R3 门闸**：未选齐 LLM + 主生图时 **发送不可用** + 一句说明；附件入口占位（R5：**不要**画「参考图张数上限」配置项）。
5. **覆盖层**：**R15** 确认（主操作「允许」、次要「拒绝或跳过」、**无倒计时**）；**R16** 与对应工具步骤 **内联** 可见，不依赖「仅靠全局 toast」的暗示。

### 3. Interaction & motion（标注级）

1. **发送**：从不可用到可用时，稿上标注 **短暂强调**（如透明度/描边变化意向）。
2. **工具步骤**：展开/收起 **有过渡**（标「有过渡」即可）。
3. **R15**：打开时 **主操作获焦点**；关闭后焦点回到触发处（无障碍意向，稿上可文字标注）。
...
| R6 | 用量条「有数据帧」与「无数据 / 隐藏帧」 |
| R15 | 独立画板或覆盖层：文案槽 + 双动作 |
...
3. **R15**：稿面表现为 **模态覆盖**（或全屏等价物），**两枚主按钮** 语义清晰；禁止稿面出现「倒计时自动点」。
...
- [ ] **D1. 对话主界面线框与关键状态（`design/layout.pen`）**
...
- 使用 **画板/帧** 组织：默认对话、选择器未齐、工具展开、R15 打开、工具失败、用量隐藏、用量显示、窄屏侧栏方案（注释）。
...
- Happy path：审阅者能指出 **侧栏、顶栏三选、时间线、输入区、R15** 五块主区域。
- Edge case：窄屏 **侧栏意图** 有注释或第二帧。
- Edge case：**R6** 「无用量」与「有用量」至少一种对比呈现。

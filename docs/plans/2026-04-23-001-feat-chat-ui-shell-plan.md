...
3. **主栏中部**：用户消息与助手消息 **一律左对齐**（不做左右对聊气泡）；用 **不同背景色** 区分 user input 与 agent output；**可折叠工具步骤**（工具名 + 进行中 / 成功 / 失败）：**入参/出参仅一个 `toolParams` 容器，内嵌单行文本组件**；行首 ▸ / ▾ 与 JSON 在同一组件内切换（收敛超长省略 / 展开全量），**不**再拆成两个并列文本节点；**不**用 LLM 摘要；**对话内图片占位**（R18：稿中用矩形/示意即可）。**注：** 上述 `toolParams` 容器与 ▸/▾ 切换为**视觉建议（设计交付层面）**，非产品级行为约束；产品级工具时间线行为仅由 R8（直接用内部 tool 名）规定，工程实现可在保留"入参出参可读、可展开"语义前提下选择不同呈现形式。
4. **主栏底部**：**输入区上方一行** **LLM | 主生图（可空） | 次生图（可空）**；其下为输入区、主发送动作；**R3 门闸**：**仅未选 LLM** 时发送不可用 + 一句说明；主生图未选时发送仍可用，但稿中需有一帧示意 Agent 在用户表达生图意图时**通过文本回复告知能力缺失**（非弹窗、非阻塞——见 R9 工具暴露规则）；附件入口占位（R5：**不要**画「参考图张数上限」配置项；超张数时稿中应有一帧调用前阻塞提示）。**R19 发送 / 停止**：发送按钮按状态切换为 **stop**——空闲时「发送」、Agent 工作中（含等待 R15 确认时）「停止」。
5. **工具卡内联**：**R15** 确认出现在 **对应工具调用卡片内**（非弹窗 / 非全屏遮罩）：**「确认」 / 「拒绝」二元按钮**（不区分「拒绝」与「跳过」）、**无倒计时**；**R16** 仍在工具步骤 **内联** 可见，不依赖「仅靠全局 toast」的暗示。
...
| 来源 | 设计稿中如何体现 |
| ---- | ---------------- |
| R3 | 三选择器、新会话空选态；**仅 LLM 未选时**发送不可用帧；主生图未选时 Agent 文本告知缺失帧（非阻塞） |
| R6 | 本对话 **LLM 上下文** **圆环**（附 **hover 详情**）「有数据帧」与「无数据 / 隐藏帧」（非 Provider 总用量） |
| R9 | 主未选 → 无生图工具暴露 / 主已选 → 暴露主 / 主与次均已选 → 暴露主 + 次（稿中可不展开工具列表，但应在文案中体现） |
| R15 | 工具调用卡片 **内联**：文案槽 +「确认」/「拒绝」二元按钮；独立画板仅作评审示意（非弹窗） |
| R16 | 工具步骤 **错误** 态；可与时间线并列 |
| R19 | 输入区主按钮在「发送」与「停止」之间按状态切换；至少一帧展示「停止」态 |
| F1 / R17 | 设置视图：LLM / 生图 / Search 分区；Search 按 type 分支字段（如 Brave 不显示 baseURL 概念） |
| F2 | 多工具步骤在时间线上 **可读、可扫** |
| R17 / R18 | Search 区域占位；对话内多图占位（不要求稿中写 URL 规则） |

**Origin actors:** A1（终端用户）, A2（Agent，间接体现）  
**Origin flows:** F1（配置）, F2（对话与工具）, F3（确认与失败）  
**Origin acceptance examples:** 设计评审时优先核对 **AE3、AE4、AE5（R15 确认二元按钮）、AE6（R16 工具失败）、AE7（R3 持久化）、AE8（R3 / R9 无生图工具时 Agent 文本告知）、AE11（R19 中断 / send-stop 同按钮）** 的 **可视形态**（非 API 行为）。AE9 / AE10 偏运行时行为，设计稿可不展开。
...
3. **R15**：稿面表现为 **工具卡内联面板**（与 `image.generate` 等步骤同卡），**「确认」/「拒绝」两枚按钮** 语义清晰（不区分「拒绝」与「跳过」）；**禁止**弹窗 / 遮罩层方案；禁止稿面出现「倒计时自动点」。
...
- 使用 **画板/帧** 组织：默认对话、**仅 LLM 未选时发送禁用**、**仅选 LLM 时 Agent 文本告知能力缺失**（R3 / R9 / AE8）、工具展开、R15 内联待批准（确认 / 拒绝二元按钮）、工具失败、用量隐藏、用量显示、撞窗口提示、**发送 / 停止按钮态切换**（R19 / AE11）、窄屏侧栏方案（注释）。
...
- Covers AE5：R15 **「确认」/「拒绝」二元按钮**、无倒计时示意。
- Covers AE6：工具失败 **附着于步骤** 的可读布局。
- Covers AE8：仅选 LLM、表达生图意图时 Agent 文本告知能力缺失帧（非弹窗、非阻塞）。
- Covers AE11：发送区按钮在「发送」/「停止」状态切换至少一帧示意。
...
3. 需求表 **R3 / R6 / R9 / R15 / R16 / R19 / F1 / F2** 在稿中 **可指出对应区域**。
